...
(function () {
  ...
  async function loadSettings() {
    ...
    el('settings-body').innerHTML = `
      ...
      <div class="card" style="margin-bottom:18px" id="carte-2fa"><div class="ch"><h3>Double authentification</h3><span class="hint">votre compte</span></div><div class="cb" id="corps-2fa"><div class="loading">Chargement…</div></div></div>
  ...
  //
  // Trois états : inactive, appairage en cours, active. L'appairage se termine
  // par un premier code — sans quoi une clé mal recopiée enfermerait dehors.
  // ========================================================================

  /** Ouvre la modale du panneau avec un contenu libre (même motif que les autres). */
  function modale2fa(html) {
    modal.classList.remove('wide');
    modal.innerHTML = html;
    backdrop.classList.add('show');
  }

  const poster2fa = (chemin, corps) => fetchJson('/api/admin/2fa' + chemin, {
    method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(corps || {}),
  });

  async function dessiner2fa() {
    const corps = el('corps-2fa');
    if (!corps) return;
    let etat;
    try { etat = await fetchJson('/api/admin/2fa'); }
    catch (e) { corps.innerHTML = `<div class="alert alert-err">${escapeHtml(e.message)}</div>`; return; }

    if (!etat.active) {
      corps.innerHTML = `
        <p style="font-size:.9rem;color:var(--ink-2);margin-bottom:12px">Un mot de passe seul protège aujourd'hui l'accès à toutes les demandes et au journal. La double authentification ajoute un code à six chiffres, renouvelé toutes les 30 secondes sur votre téléphone.</p>
        <button class="btn btn-primary" id="btn-2fa-preparer" type="button">Activer</button>`;
      el('btn-2fa-preparer').addEventListener('click', preparer2fa);
      return;
    }
    corps.innerHTML = `
      <p style="font-size:.9rem;margin-bottom:6px"><span class="badge st-terminee">Active</span></p>
      <p style="font-size:.86rem;color:var(--muted);margin-bottom:14px">Il vous reste <b>${etat.codesRestants}</b> code(s) de secours.${etat.codesRestants <= 2 ? ' <span style="color:var(--warn)">Pensez à en générer de nouveaux.</span>' : ''}</p>
      <div style="display:flex;gap:10px;flex-wrap:wrap">
        <button class="btn btn-ghost" id="btn-2fa-codes" type="button">Nouveaux codes de secours</button>
        <button class="btn btn-ghost" id="btn-2fa-off" type="button">Désactiver</button>
      </div>`;
    el('btn-2fa-codes').addEventListener('click', () => demanderMotDePasse(
      'Nouveaux codes de secours',
      'Les anciens codes seront invalidés. Confirmez avec votre mot de passe.',
      async (password) => montrerCodes((await poster2fa('/codes', { password })).codesSecours)
    ));
    el('btn-2fa-off').addEventListener('click', () => demanderMotDePasse(
      'Désactiver la double authentification',
      'Votre compte ne sera plus protégé que par son mot de passe. Confirmez avec ce mot de passe.',
      async (password) => { await poster2fa('/desactiver', { password }); toast('Double authentification désactivée'); dessiner2fa(); }
    ));
  }

  async function preparer2fa() {
    let prep;
    try { prep = await poster2fa('/preparer'); }
    catch (e) { return toast(e.message, true); }
    modale2fa(`
      <h3 style="margin-bottom:6px">Activer la double authentification</h3>
      <p style="font-size:.88rem;color:var(--muted);margin-bottom:16px">Dans Microsoft Authenticator, Google Authenticator ou FreeOTP : « Ajouter un compte » → « Saisir une clé manuellement ».</p>
      <div class="field"><label>Nom du compte</label>
        <div class="cle-2fa"><span class="v">${escapeHtml(prep.uri.split(':')[1].split('?')[0].replace(/%40/g, '@'))}</span><button type="button" data-copy="${escapeHtml(prep.uri.split(':')[1].split('?')[0].replace(/%40/g, '@'))}">Copier</button></div>
      </div>
      <div class="field"><label>Clé secrète</label>
        <div class="cle-2fa"><span class="v mono">${escapeHtml(prep.secretLisible)}</span><button type="button" data-copy="${escapeHtml(prep.secret)}">Copier</button></div>
      </div>
      <div class="field" style="margin-top:14px">
        <label for="code-2fa">Saisissez le code affiché par l'application</label>
        <input type="text" id="code-2fa" inputmode="numeric" maxlength="7" placeholder="123456" class="inp champ-code" />
      </div>
      <p class="alert alert-err" id="err-2fa" style="display:none"></p>
      <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:16px">
        <button class="btn btn-ghost" id="annuler-2fa" type="button">Annuler</button>
        <button class="btn btn-primary" id="valider-2fa" type="button">Vérifier et activer</button>
      </div>`);
    for (const b of document.querySelectorAll('[data-copy]')) {
      b.addEventListener('click', () => { navigator.clipboard.writeText(b.dataset.copy); toast('Copié'); });
    }
    el('annuler-2fa').addEventListener('click', closeModal);
    el('valider-2fa').addEventListener('click', async () => {
      const erreur = el('err-2fa');
      erreur.style.display = 'none';
      try {
        const out = await poster2fa('/activer', { code: el('code-2fa').value });
        montrerCodes(out.codesSecours);
      } catch (e) {
        erreur.textContent = e.message;
        erreur.style.display = 'block';
      }
    });
    setTimeout(() => el('code-2fa') && el('code-2fa').focus(), 50);
  }

  /** Les codes de secours ne sont affichés QU'ICI : ils ne sont stockés que hachés. */
  function montrerCodes(codes) {
    modale2fa(`
      <h3 style="margin-bottom:6px">Vos codes de secours</h3>
      <p style="font-size:.88rem;color:var(--muted);margin-bottom:14px">Chacun ne sert qu'une fois, en remplacement du code de l'application si vous perdez votre téléphone. <b>Ils ne seront plus jamais affichés</b> : conservez-les hors de ce portail.</p>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-family:var(--mono);font-size:.95rem">
        ${codes.map((c) => `<div style="padding:8px 10px;background:var(--surface-2);border:1px solid var(--line);border-radius:8px;text-align:center">${escapeHtml(c)}</div>`).join('')}
      </div>
      <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:18px">
        <button class="btn btn-ghost" id="copier-codes" type="button">Tout copier</button>
        <button class="btn btn-primary" id="fermer-codes" type="button">J'ai noté ces codes</button>
      </div>`);
    el('copier-codes').addEventListener('click', () => { navigator.clipboard.writeText(codes.join('\n')); toast('Codes copiés'); });
    el('fermer-codes').addEventListener('click', () => { closeModal(); dessiner2fa(); });
  }

  /** Confirmation par mot de passe : un poste laissé ouvert ne doit pas suffire. */
  function demanderMotDePasse(titre, explication, action) {
    modale2fa(`
      <h3 style="margin-bottom:6px">${escapeHtml(titre)}</h3>
      <p style="font-size:.88rem;color:var(--muted);margin-bottom:14px">${escapeHtml(explication)}</p>
      <div class="field"><label for="mdp-confirm">Mot de passe</label>
        <input type="password" id="mdp-confirm" class="inp" autocomplete="current-password" /></div>
      <p class="alert alert-err" id="err-confirm" style="display:none"></p>
      <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:16px">
        <button class="btn btn-ghost" id="annuler-confirm" type="button">Annuler</button>
        <button class="btn btn-primary" id="ok-confirm" type="button">Confirmer</button>
      </div>`);
    el('annuler-confirm').addEventListener('click', closeModal);
    el('ok-confirm').addEventListener('click', async () => {
      const erreur = el('err-confirm');
      erreur.style.display = 'none';
      try { await action(el('mdp-confirm').value); }
      catch (e) { erreur.textContent = e.message; erreur.style.display = 'block'; }
    });
    setTimeout(() => el('mdp-confirm') && el('mdp-confirm').focus(), 50);
  }
})();
